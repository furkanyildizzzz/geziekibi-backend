...
import { IJsonWebTokenService } from '../interfaces/IJsonWebToken';
...
export class JsonWebTokenService implements IJsonWebTokenService {
  ...
  async encode(payload: Object, isAccessToken: boolean): Promise<string> {
    return await sign(payload, isAccessToken ? this.JWT_ACCESS_TOKEN_SECRET : this.JWT_REFRESH_TOKEN_SECRET, {
      expiresIn: isAccessToken
        ? process.env.JWT_ACCESS_TOKEN_EXPIRATION_TIME || '90000'
