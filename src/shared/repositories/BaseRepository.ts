import { DeepPartial, EntityMetadata, FindOptionsWhere, Repository, SelectQueryBuilder } from 'typeorm';
...
export class BaseRepository<T extends BaseEntity> implements IBaseRepository<T> {
  ...
  constructor(protected readonly unitOfWork: UnitOfWork, entity: { new (): T }) {
    this.entity = entity;
  }
  ...
  async getAll(relations = [], applyFilter = true, where?: FindOptionsWhere<T>): Promise<T[]> {
  ...
  async getById(id: number, relations = [], applyFilter = true): Promise<T | null> {
    try {
      ...
      const queryBuilder = repo.createQueryBuilder('entity').where('entity.id = :id', { id });
  ...
  async getByIds(ids: number[], relations = [], applyFilter = true): Promise<T[] | null> {
    try {
      ...
      const queryBuilder = repo.createQueryBuilder('entity').where('entity.id IN (:...ids)', { ids });
  ...
  async deleteMultiple(ids: number[]): Promise<void> {
    ...
    if (user.role != 'ADMINISTRATOR') {
      const ownedEntities = await repo
        .createQueryBuilder('entity')
  ...
  public async findOneByField<K extends keyof T>(field: K, value: T[K], applyFilter = true): Promise<T | null> {
    const repo = await this.getRepository();
    const queryBuilder = repo.createQueryBuilder('entity').where(`entity.${field as string} = :value`, { value });
